...
export const getStaticProps = async ({ params: { slug } }) => {
  // fetch product
  const query = `*[_type == "product" && slug.current == '${slug}'][0]`; //get the individuala product
  const productsQuery = '*[_type == "product"]';

  const product = await client.fetch(query); //to fecth individual product
...
//When use getStaticProps, you must also use getStaticPaths
